...
def render_valuation_peer_card(report: ResearchReport, output: str | Path) -> Path:
    ...
    ax.text(0.05, 0.966, f"{report.symbol} · Değerleme ve Rakip Analizi", fontsize=22, fontweight="bold", color=TEXT, va="top")
    ax.text(
        0.05,
        0.936,
    ...
    keys = (
        "pe", "pb", "ev_ebitda", "ev_sales", "ps",
        "p_fcf", "peg", "dividend_yield", "earnings_yield", "fcf_yield",
    )
    x0, y0 = 0.05, 0.875
    card_w, card_h = 0.168, 0.090
    gap_x, gap_y = 0.015, 0.016
    for idx, key in enumerate(keys):
        ...
        ax.text(x + 0.012, y + 0.067, LABELS[key], fontsize=8.3, fontweight="bold", color=TEXT, transform=ax.transAxes)
        ax.text(x + 0.012, y + 0.038, _fmt(value, key), fontsize=13, fontweight="bold", color=tone, transform=ax.transAxes)
        ax.text(x + 0.012, y + 0.013, phrase, fontsize=6.4, color=MUTED, transform=ax.transAxes)

    bench_y = 0.585
    _box(ax, 0.05, bench_y, 0.90, 0.205)
    ax.text(0.072, bench_y + 0.172, "Sektör Karşılaştırması", fontsize=13, fontweight="bold", color=TEXT, transform=ax.transAxes)
    ax.text(0.072, bench_y + 0.148, "Metrik", fontsize=7.4, fontweight="bold", color=MUTED, transform=ax.transAxes)
    ax.text(0.36, bench_y + 0.148, report.symbol, fontsize=7.4, fontweight="bold", color=MUTED, transform=ax.transAxes, ha="right")
    ...
    for key in ("pe", "pb", "ev_ebitda", "ev_sales", "roe"):
        ...
        ax.text(0.36, row_y, _fmt(target, key), fontsize=8.3, fontweight="bold", color=tone, transform=ax.transAxes, ha="right")
        ax.text(0.53, row_y, _fmt(median, key), fontsize=8.3, color=TEXT, transform=ax.transAxes, ha="right")
        ax.text(0.69, row_y, "—" if percentile is None else f"%{percentile:.0f}", fontsize=8.3, color=TEXT, transform=ax.transAxes, ha="right")
    ...
    ax.text(0.072, peers_y + peers_h - 0.035, "Rakip / Sektör Tablosu", fontsize=13, fontweight="bold", color=TEXT, transform=ax.transAxes)
    headers = (("Hisse", 0.075), ("F/K", 0.34), ("PD/DD", 0.46), ("FD/FAVÖK", 0.60), ("FD/Satış", 0.75), ("ROE", 0.91))
    for label, x in headers:
        ax.text(x, peers_y + peers_h - 0.067, label, fontsize=7.2, fontweight="bold", color=MUTED, transform=ax.transAxes, ha="right" if x > 0.1 else "left")
    row_y = peers_y + peers_h - 0.098
    peers = list(peer.get("peers", ()))[:8]
    if not peers:
        ax.text(0.075, row_y, "Karşılaştırılabilir rakip verisi bulunamadı.", fontsize=9, color=MUTED, transform=ax.transAxes)
    for item in peers:
    ...
    ax.text(0.05, 0.153, f"Göreli değerleme skoru {score_text} · kapsam {coverage_text}", fontsize=10.5, fontweight="bold", color=TEXT, transform=ax.transAxes)
    ax.text(0.05, 0.115, str(valuation.get("note", "")), fontsize=8.0, color=MUTED, transform=ax.transAxes, wrap=True)
    ax.text(0.05, 0.072, str(valuation.get("peg_note", "")), fontsize=7.2, color=MUTED, transform=ax.transAxes, wrap=True)
    ax.text(
        0.05,
        0.025,
