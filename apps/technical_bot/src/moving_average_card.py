...
price relation, local slope and one summary line per horizon.
"""
...
def build_moving_average_snapshot(symbol: str) -> dict[str, Any]:
    ...
    if frame.empty or len(frame) < 240:
    ...
    return {"symbol": ticker, "price": price, "rows": rows, "summaries": summaries}
...
def render_moving_average_card(symbol: str, output: Path) -> tuple[Path, dict[str, Any]]:
    ...
    fig.text(0.06, 0.925, f"Günlük SMA · Son fiyat {snapshot['price']:,.2f} TL", color=MUTED, fontsize=9.5, va="top")
    ...
    for row in snapshot["rows"]:
        ...
        value_text = "—" if row["value"] is None else f"{row['value']:,.2f}"
        ax.text(0.38, y, value_text, color=CYAN, fontsize=10.0, va="center")
    ...
    fig.text(0.94, 0.025, "Değer + fiyat konumu + 3 günlük MA eğimi · otomatik AL/SAT değildir", color=MUTED, fontsize=7.7, ha="right")
